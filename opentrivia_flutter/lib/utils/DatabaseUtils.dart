...
class DatabaseUtils {
  ...
  Future<void> getAvversario(String difficolta, String partita, Function(bool, String, String) callback) async {
  ...
  Future<void> getRispCorrette(String difficolta, String partita, Function(int, int) callback) async {
    DatabaseReference giocatoriRef = FirebaseDatabase.instance.ref().child("partite").child(difficolta).child(partita).child("giocatori");
    String uid = FirebaseAuth.instance.currentUser?.uid ?? "";
    int risposte1 = 0;
    int risposte2 = 0;
    ...
    try {
      ...
      if (giocatore.value != null) {
        for (DataSnapshot topic in giocatore.children) {
          if (topic.hasChild("risposteTotali")) {
            String giocatore1 = giocatore.key.toString();

            // PER ME
            if (giocatore1 == uid) {
              var risposteCorrette = topic.child("risposteCorrette").value;
